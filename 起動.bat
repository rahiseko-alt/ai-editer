...
echo サーバーを起動しています...
start "KOSM サーバー" cmd /k node server/index.mjs

echo 編集ワーカーを起動しています...
start "KOSM 編集の進行" cmd /k node server/job-worker.mjs
...
if errorlevel 1 (
  ...
  echo 「KOSM サーバー」のウィンドウに出ているメッセージを確認してください。
  echo.
  pause
  exit /b 1
)
...
echo 編集の進み具合は「KOSM 編集の進行」のウィンドウに出ます。
