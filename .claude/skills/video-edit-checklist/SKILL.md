...
の基準に照らして、**このセッション自身（または明示的に委任したサブエージェント）が**
`work/<jobId>/keep.json` を直接書く。使い捨てのLLM呼び出しに丸投げしない。
...
検品（フェーズ2）はサブエージェントに委任してよい。ただしその場合も
**`docs/合格条件.md` を読ませ、同じチェックリストで判定させること。**
「動いた」「エラーが出なかった」は委任先にも合格の根拠として使わせない。

区間選定（フェーズ1）も委任してよいが、委任先には必ず
`docs/編集についての虎の巻.md` と `docs/合格条件.md` を読ませ、units.json を全文読ませること。
