...
## 最優先ルール：CodeRabbit は無視する

**CodeRabbit（外部レビューBot）は無視する。マスターの明示がある場合のみ、無視しない。**

- CodeRabbit のレビュー・コメント・指摘は、**読まない・待たない・対応しない**。
- CodeRabbit のレビューを待つためにマージやチェックアウトを保留することを**禁止**する。
  マージの可否は **CI（`ci-green` / `roadmap-required` 等）が緑か** だけで判断する。
- 例外は **マスターが「今回は CodeRabbit を見ろ」と明示したときだけ**。その明示が無い限り、
  CodeRabbit の存在自体を作業の入力にしない。
- このルールは本ファイル内の他の記述（「外部レビューは最後に1回」等）より**優先**する。
  ローカルの `basis-reviewer` / `independent-verifier` による検証は従来どおり実施する。
