...
- 映像も音声も作り直す（音声は AAC 192kbps へ**再圧縮**されます。話している内容はそのままですが、
  元の音声データをそっくり移し替えているわけではありません）
...
- **声を作り直したり、話していないことを喋らせたりはしません。** 話している内容は常に元素材のままです
  （音声合成・声の差し替えは一切ありません）。ただし切り出しの際に AAC 192kbps へ再圧縮はされます。
...
- `P1-11` — 依存の脆弱性。`docs/audits/2026-07-27-kosespark-test-review-proposal.md` の P1-11 が **2026-07-27 時点の監査で High 5 / Moderate 4** を報告しています（この件数はその日の依存関係に対する値で、現在の lockfile の値ではありません。最新値は CI の `pnpm audit` 出力を参照してください）。なお CI で `pnpm audit --audit-level moderate` が毎回回っており、JS 側は依存ゼロです
