...
- 効いているかは、**文字起こしの最中に、画面左上のロゴの右へ「Turbo」と出るか**で分かる
  （2026-08-17 変更。以前はここに処理先の説明文が出ていたが、マスター指示で説明文は廃止した）。
  出るのは文字起こしの間だけで、終われば消える。鍵が無いときは出ない。
...
### ステップ5：編集の前に台本の確認画面が出るので、内容を見て承認する

動画を選んで開始すると、**書き出し（編集作業）が始まる前に一度止まり**、
「**この台本で書き出します**」という確認画面が出る。**ここで承認するまで書き出しは始まらない。**

- 画面には、AI が「使う」と判断した場面が一覧で出る。**内容を読んで確かめる。**
- 使わない場面はチェックを外す（外した場面は書き出しに入らない）。
- そのままで良ければ「**この内容で書き出す**」を押す。ここで初めて編集が始まる。
- 中止したいときは「**やめる**」を押す。そのジョブは捨てられる（ターミナルはそのままで良い）。

> この確認画面は**既定で出る**（起動コマンドに何も足す必要は無い）。マスターの指示
> 「文字起こし→台本→…→**ユーザーに提示→ユーザー承認を経て編集作業の開始**」がそのまま
> 通常の動作になっているためである。確認画面が出ないまま書き出しが始まったら、それは異常。
> ステップ6の手順で出力を貼って AI に伝える。
>
> 承認を送る人がいない無人実行（自動で回すだけの用途）に限り、起動時に
> `VS_REQUIRE_APPROVAL=off` を指定すると確認画面を出さずに走り切る。**マスターの手元の
> 手順では使わない**（ステップ2のブロックには入れない）。

### ステップ6：うまくいかないときに AI へ渡すもの
