...
- 画面には、文字起こしの**全文**がほぼ画面いっぱいに出る。AI が使うと決めた所は白、
  使わない所は濃いグレーで区別される。**内容を読んで確かめる。**
- 白い所をクリック（タップ）すると使わない側へ落ちる。もう一度クリックすれば戻る。
  ドラッグでまとめて操作もできる（2026-08-17 変更。以前は場面ごとのカードだった）。
