...
export async function renderClip(p) {
  ...
  //
  // === J カット / L カット（虎の巻 §3-6・§8-K）は「やらない」判断（2026-08-17） ===
  // 【実測（再現）】合成素材（[0,4)=赤い画+1000Hz / [4,8)=青い画+300Hz）から、製品経路
  // （renderSegment → src/concat.mjs concatClips）で部品A=[1,3]・部品B=[5,7] を焼いて連結し、
  // 出力の中で画が青へ変わった時刻と音が300Hzへ変わった時刻を別々に実測した:
  //   画=2.033秒（30fps の61コマ目） / 音=2.024秒 → 差 0.010秒（1コマ=33ms 未満）。
  // ＝画と音は同じ瞬間に切れており、J/L カット（音か画を数百ms ずらす）は存在しない。
  //
  // 【なぜここでは直さないか】J/L カットは「1つのつなぎ目で、音と画が別の素材位置から来る」
  // ことが本体で、そのためには 1 クリップの中で素材の2箇所を跨ぐ音声（または映像）が要る。
  // いまの作りは逆に「1つの窓（-ss/-t）で映像と音声を同時に切る」ことを土台にしていて、
  // それ自体が絵と音の原点を揃えるための設計（上の A/V 同期の節）。ずらすには次が要る:
  //   1. renderClip の音声経路に「窓を2つ持つ」概念を入れる。詰める経路の式は
  //      src/trim-plan.mjs の buildTrimFilters が持つので、詰める側・詰めない側の両方に入る。
  //   2. または連結側で重ねる。src/concat.mjs は concat demuxer + `-c copy` の突き合わせで、
  //      重ね合わせ（acrossfade / amix）ができない。filter_complex 方式へ作り直すことになり、
  //      無劣化・高速（copy）の利点を捨てる。
  //   3. 字幕(ASS)はクリップ相対の時刻で焼くので、音をずらすなら字幕の時間軸も同じだけ動かす。
  //   4. ずれた区間ぶん、詰める処理（planTrim）の keep とコマ番号の対応も引き直しになる。
  // 出荷経路の A/V 同期はこのリポジトリで最も手厚く実測して固めてきた性質（上の節）なので、
  // 「つなぎ目を自然にする」ために土台を崩す取引は割に合わない、という判断。
  //
  // 【やるときの前提は整っている】§3-6 は「重ねてよいのは自然なポーズかブレスの上だけ」と
  // 決めている。2026-08-17 に topic 経路の切り口を実測した無音の内側へ収めた
  //（src/breath-handles.mjs planTopicHandles）ので、つなぎ目は無音の中にある。
  // digest の部品も planBreathParts が同じ性質を持つ。＝重ねる場所は既に確定できる状態にある。
  //
