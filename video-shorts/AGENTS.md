...
- クラウド / ホスティング: 不要（客の PC 上でローカル動作。kosespark のサーバ・アカウントを介さない）
...
- パッケージ / 依存管理: pnpm workspace（本パッケージ自体は npm 依存ゼロ）。外部バイナリとして ffmpeg/ffprobe が必須（別途導入）
...
- 全工程キーレスが原則。Groq を使う場合のみ客自身の API キーを使用（kosespark が代理・再販しない）。
