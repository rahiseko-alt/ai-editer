...
//   音声(a:0)の尺が 5.200秒（78コマ＝78/15秒。2026-08-16 の余韻0.3秒導入後の実測）、素材TA-S（同じ語・同じ言い淀みを 0.10秒＝
//   しきい値未満の間隙で並べ直したもの）では 4.933秒（74コマ）になり、どちらも出力の中で
//   声が鳴っている区間の位置が、凍結した位置と一致する。両者の『詰めた量』の差 2.532秒 が、
//   長い無音を詰めた寄与にあたる。」
//
// ── なぜこの測り方か ──────────────────────────────────────────
// (1) なぜ尺だけで判定しないか
//     「尺が期待どおり」は、末尾を切り落としただけの実装でも成立する（空虚に真）。
//     実際、この検査の変異実験 M2（一切詰めずに -t を 5.200秒 にするだけ）は尺の判定を通る。
...
// 詰める無音の直前に「残る発言」があるときだけ、その終端から 0.3秒（＝15コマ/秒で4.5コマ、
// 端の丸めで4コマ）を残すようになった。この素材で余韻が付くのは、直前が残る語である無音
// 3箇所（1つめ・2つめ・3つめの語の直後）。4つめの語の後ろは素材の終わりで、末尾の無音が
// 無いため余韻は生じない。よって各区間の終端が 4コマずつ後ろへ伸びる。
//   [20,39]→[20,43] / [65,81]→[65,86] / [108,125]→[108,130] / [154,166] は据え置き
//   合計 64→78コマ = 78/15 = 5.2 秒
// 以下はすべて製品経路(probeSize→snapStart→planTrim→renderClip)を通した実測値である。
const TA_KEEP_FRAMES = [[20, 43], [65, 86], [108, 130], [154, 166]];
const TA_EXPECT_SEC = 78 / 15;
...
// 素材TA-S は変更なし。間隙 0.10秒 はしきい値 0.20秒 未満で元から詰める対象ではなく、
// 余韻(0.3秒)を置く以前に詰める区間そのものが無いため、余韻の規則の影響を受けない。
...
// 2026-08-16: 余韻 0.3秒 が3箇所に入ったぶん、2区間目以降が後ろへずれる（実測）。
const TA_EXPECT_GROUPS = [
  [0.020, 0.852], [1.554, 2.235], [2.957, 3.708], [4.430, 4.801],
];
...
// 実測(2026-08-16 の余韻導入後): TAは5.866秒詰め、TA-Sは3.334秒詰め、差2.532秒。
// 設計上の無音 0.5秒×7＝3.500秒 から、余韻として残した 0.3秒×3＝0.900秒 を差し引いた
// 2.600秒 に対し、差 0.068秒 はコマ境界へ丸めたぶん（余韻を入れる前と同水準）。
const SILENCE_CONTRIB = 2.532;
...
try {
  ...
  t(`A: 素材TA の出力が、長い無音と言い淀みだけを取り除いたものになっている（${TA_EXPECT_SEC.toFixed(3)}秒・78コマ）`, () => {
    const bad = judge(outTA, TA_EXPECT_SEC, 78, TA_EXPECT_GROUPS);
  ...
  //     四捨五入した値)から、対応する正解区間(TA_KEEP_FRAMES)と同じ幅(23/21/22/12コマ)だけ
  //     取る。コマ番号で [0,23] [46,67] [89,111] [133,145] ＝ 23+21+22+12 ＝ 78コマ ＝ 5.200秒
  ...
  await runProduct(TA, sizeTA, wordsTA, TA_DUR, mutTA.wrongPlace, {
    keep: [[0, 23], [46, 67], [89, 111], [133, 145]].map(([a, b]) => ({ start: a / FPS, end: b / FPS })),
  });
  ...
  t("変異: 一切詰めない実装は落ちる（尺も声の位置も合わない）", () => {
    const bad = judge(mutTA.none, TA_EXPECT_SEC, 78, TA_EXPECT_GROUPS);
  ...
  t("変異: 末尾を切るだけの実装は落ちる（尺は合うが、声の位置が合わない）", () => {
    ...
    const bad = judge(mutTA.tail, TA_EXPECT_SEC, 78, TA_EXPECT_GROUPS);
  ...
  t("変異: 尺もコマ数も合うが残す場所が違う実装は落ちる（言い淀みのほうを残した実装）", () => {
    ...
    assert.ok(Math.abs(d - TA_EXPECT_SEC) <= TOL_SEC,
      `この変異は尺が合っている前提の実験だが、実=${d.toFixed(3)}秒だった`);
    assert.strictEqual(videoFrames(mutTA.wrongPlace), 78, "この変異はコマ数も合っている前提");
    const bad = judge(mutTA.wrongPlace, TA_EXPECT_SEC, 78, TA_EXPECT_GROUPS);
    assert.ok(bad.length > 0, "残す場所が違うのに合格した");
  });
  t("変異: コマ境界へ揃えない実装は落ちる（＝fps を凍結する必要があることの実証）", () => {
    const bad = judge(mutTA.nofps, TA_EXPECT_SEC, 78, TA_EXPECT_GROUPS);
